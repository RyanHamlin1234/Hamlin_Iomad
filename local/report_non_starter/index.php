...
iomad::require_capability('local/report_non_starter:view', $context);
...
// Url stuff.
$url = new moodle_url('/local/report_non_starter/index.php');
$dashboardurl = new moodle_url('/local/iomad_dashboard/index.php');

// Page stuff:.
$strcompletion = get_string('pluginname', 'local_report_non_starter');
$PAGE->set_url($url);
$PAGE->set_pagelayout('report');
$PAGE->set_title($strcompletion);
$PAGE->set_heading($SITE->fullname);
$PAGE->requires->css("/local/report_non_starter/styles.css");
...
$url = new moodle_url('/local/report_non_starter/index.php', $params);

// Get the appropriate list of departments.
$selectparams = $params;
$selectparams['courseid'] = 0;
$selecturl = new moodle_url('/local/report_non_starter/index.php', $selectparams);
...
if (empty($dodownload) && empty($showchart) && !$sendemail) {
    echo "<h3>".get_string('coursesummary', 'local_report_non_starter')."</h3>";
    if (!empty($courseid)) {
        // Navigation and header.
        echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("downloadcsv", 'local_report_non_starter'));
        $options['charttype'] = 'summary';
        $options['dodownload'] = false;
        //echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("summarychart", 'local_report_non_starter'));
		$options['charttype'] = '';
        $options['sendemail'] = true;
        echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("sendreminderemail", 'local_report_non_starter'));
	} else {
        $options['charttype'] = 'summary';
        $options['dodownload'] = false;
        //echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("summarychart", 'local_report_non_starter'));
        ...
        echo $OUTPUT->single_button($alluserslink, get_string("allusers", 'local_report_non_starter'));
    }
...
$coursecomptable->head = array(
    get_string('coursename', 'local_report_non_starter'),
    //get_string('numallocated', 'local_report_non_starter'),
    //get_string('unused', 'local_report_non_starter'),
    ' ',
);
$coursecomptable->align = array('left', 'center'); //$coursecomptable->align = array('left', 'center', 'center', 'center');
...
if (!empty($dodownload)) {
    ...
    header("Content-Disposition: attachment; filename=\"non_starter_report.csv\"");
    header("Expires: 0");
...
foreach ($courseinfo as $id => $coursedata) {
    ...
    $coursecomptable->data[] = array(
       ...
       // $coursedata->numunused,
        '<a class="btn" style="margin:2px" href="' . $courseuserslink . '">' . get_string('usersummary', 'local_report_non_starter') . '</a>&nbsp;',
        //'<a class="btn" style="margin:2px" href="' . $coursechartlink . '">' . get_string('cchart', 'local_report_non_starter') . '</a>',
    );
...
if (!empty($charttype)) {
    ...
    $chartdata->addPoints(array(
        get_string('notstartedusers', 'local_report_non_starter'),
        get_string('inprogressusers', 'local_report_non_starter'),
    ), 'Legend');
    ...
    if (!empty($courseid)) {
        ...
        if ($dodownload) {
            ...
            if ($confirm && confirm_sesskey()) {
                ...
                redirect(new moodle_url('/local/report_non_starter/index.php', $params));
            } else {
                echo $OUTPUT->header();
                $params['confirm'] = true;
                $params['sesskey'] = sesskey();
                $buttons = $OUTPUT->single_button(
                    new moodle_url('/local/report_non_starter/index.php', $params),
                ...
                echo $OUTPUT->box(get_string('licensesendreminder', 'local_report_non_starter'));
        ...
        if (empty($dodownload) && !$sendemail) {
            if ($courseid == 1) {
                echo "<h3>".get_string('reportallusers', 'local_report_non_starter')."</h3>";
            } else {
                echo "<h3>".get_string('courseusers', 'local_report_non_starter').$courseinfo[$courseid]->coursename."</h3>";
            }
        }
        ...
        foreach ($columns as $column) {
            if ($column == 'lastaccess') {
                ...
                $string[$column] = get_string($column, 'local_report_non_starter');
            }
            if ($sort != $column) {
                $columnicon = "";
                $columndir = "ASC";
        ...
        if (!empty($dodownload)) {
            ...
            if ($courseid == 1) {
                echo get_string('allusers', 'local_report_non_starter')."\n";
            } else {
                echo $courseinfo[$courseid]->coursename."\n";
            }
            if ($showused) {
                echo '"'.get_string('name', 'local_report_non_starter').'","'
                     .get_string('email', 'local_report_non_starter').'","'
                     .get_string('course').'","'
                     .get_string('department', 'block_iomad_company_admin').'","'
                     .get_string('lastaccess').'","'
                     .get_string('licensename', 'local_report_non_starter').'","'
					 .get_string('issuedate', 'local_report_non_starter')."\"\n";
            } else {
                echo '"'.get_string('name', 'local_report_non_starter').'","'
                     .get_string('email', 'local_report_non_starter').'","'
                     .get_string('course').'","'
        ...
        if (empty($idlist['0'])) {
            foreach ($coursedata as $user) {
				...
				if ($user->lastaccess == 0) {
                    ...
                    $datestring = date('jS M Y',$user->issuedate);
				...
				if (empty($dodownload)) {
                    ...
                    if ($showused) {
                        echo '"'.$user->fullname.
                             ...
                             '","'.$datestring.
                             '","'.$user->licensename.
							 '","'.date('jS M Y',$user->issuedate).//'","'.userdate($user->issuedate)." (".format_time(time() - $user->issuedate).")".
                             //'","'.$user->isusing.
                             "\"\n";
                    } else {
                        echo '"'.$user->fullname.
        ...
        if (empty($dodownload)) {
            ...
            if (empty($idlist['0'])) {
                echo $OUTPUT->paging_bar($totalcount, $page, $perpage, new moodle_url('/local/report_non_starter/index.php', $params));
				echo "<br />";
            }
...
if (empty($dodownload) && !empty($charttype)) {
    ...
    echo "<center><img src='".new moodle_url('/local/report_non_starter/index.php', $params)."'></center>";
}
