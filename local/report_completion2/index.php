...
iomad::require_capability('local/report_completion2:view', $context);
...
// Url stuff.
$url = new moodle_url('/local/report_completion2/index.php');
$dashboardurl = new moodle_url('/local/iomad_dashboard/index.php');

// Page stuff:.
$strcompletion = get_string('pluginname', 'local_report_completion2');
$PAGE->set_url($url);
$PAGE->set_pagelayout('report');
$PAGE->set_title($strcompletion);
$PAGE->set_heading($SITE->fullname);
$PAGE->requires->css("/local/report_completion2/styles.css");
...
$url = new moodle_url('/local/report_completion2/index.php', $params);

// Get the appropriate list of departments.
$selectparams = $params;
$selectparams['courseid'] = 0;
$selecturl = new moodle_url('/local/report_completion2/index.php', $selectparams);
...
$completiontypelist = array('0' => get_string('all'),
                            '1' => get_string('notstartedusers', 'local_report_completion2'),
                            '2' => get_string('inprogressusers', 'local_report_completion2'),
                            '3' => get_string('completedusers', 'local_report_completion2'));
...
if (empty($dodownload) && empty($showchart)) {
    echo "<h3>".get_string('coursesummary', 'local_report_completion2')."</h3>";
    if (!empty($courseid)) {
        // Navigation and header.
		if (empty($params['charttype'])) {
            echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("downloadcsv", 'local_report_completion2'));
        }
		$options['charttype'] = 'summary';
        $options['dodownload'] = false;
        //echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("summarychart", 'local_report_completion2'));
    } else {
        $options['charttype'] = 'summary';
        $options['dodownload'] = false;
        //echo $OUTPUT->single_button(new moodle_url('index.php', $options), get_string("summarychart", 'local_report_completion2'));
        ...
        echo $OUTPUT->single_button($alluserslink, get_string("allusers", 'local_report_completion2'));
    }
...
$coursecomptable->head = array(
    get_string('coursename', 'local_report_completion2'),
    get_string('numusers', 'local_report_completion2'),
    get_string('notstartedusers', 'local_report_completion2'),
    get_string('inprogressusers', 'local_report_completion2'),
    get_string('completedusers', 'local_report_completion2'),
    ' ',
);
...
foreach ($courseinfo as $id => $coursedata) {
    ...
    $coursecomptable->data[] = array(
        ...
        '<a class="btn" style="margin:2px" href="' . $courseuserslink . '">' . get_string('usersummary', 'local_report_completion2') . '</a>&nbsp;',
        //'<a class="btn" style="margin:2px" href="' . $coursechartlink . '">' . get_string('cchart', 'local_report_completion2') . '</a>',
    );
...
if (empty($charttype)) {
    if (!empty($courseid)) {
        ...
        if (empty($dodownload)) {
            if ($courseid == 1) {
                echo "<h3>".get_string('reportallusers', 'local_report_completion2')."</h3>";
            } else {
                echo "<h3>".get_string('courseusers', 'local_report_completion2').$courseinfo[$courseid]->coursename."</h3>";
            }
        }
        ...
        foreach ($columns as $column) {
            $string[$column] = get_string($column, 'local_report_completion2');
            if ($sort != $column) {
                $columnicon = "";
                $columndir = "ASC";
            } else {
        ...
        if (!empty($dodownload)) {
            ...
            if ($courseid == 1) {
                echo get_string('allusers', 'local_report_completion2')."\n";
            } else {
                echo $courseinfo[$courseid]->coursename."\n";
            }
            echo '"'.get_string('name', 'local_report_completion2').'","'
                 .get_string('email', 'local_report_completion2').'","'
                 .get_string('course').'","'
                 .get_string('department', 'block_iomad_company_admin').'","'
                 .get_string('status', 'local_report_completion2').'","'
                 .get_string('timeenrolled', 'local_report_completion2').'","'
                 .get_string('timestarted', 'local_report_completion2').'","'
                 .get_string('timecompleted', 'local_report_completion2').'","'
                 .get_string('finalscore', 'local_report_completion2')."\"\n";
            $xlsrow = 1;
        }
        ...
        if ($hascertificate) {
            $compusertable->head[] = get_string('certificate', 'local_report_completion2');
        ...
        if (empty($idlist['0'])) {
            foreach ($coursedata as $userid => $user) {
                ...
                if (!empty($user->timecompleted)) {
                    $statusstring = get_string('completed', 'local_report_completion2');
                }
                ...
                if (!empty($user->timeenrolled)) {
                    $enrolledtime = date('jS M Y', $user->timeenrolled);
                } else {
                    $enrolledtime = "";
                }
                if (!empty($user->timecompleted)) {
                    $completetime = date('jS M Y', $user->timecompleted);
                } else {
                    $completetime = "";
                }
        ...
        if (empty($dodownload)) {
            ...
            if (empty($idlist['0'])) {
                echo $OUTPUT->paging_bar($totalcount, $page, $perpage, new moodle_url('/local/report_completion2/index.php', $params));
				echo "<br />";
            }
...
if (empty($dodownload) && !empty($charttype)) {
    ...
    echo "<center><img src='".new moodle_url('/local/report_completion2/index.php', $params)."'></center>";
}
